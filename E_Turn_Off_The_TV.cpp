...
class Segment_Tree{
   ...
   public:
    ...
    void push(lli id,lli l,lli r){
        ...
        //    t[2*id].mini+=t[id].lazy;
           t[2*id+1].lazy+=t[id].lazy;
        //    t[2*id+1].mini+=t[id].lazy;
           }
        }
        t[id].lazy=0;
    }
...
Segment_Tree st(m);
fr(i,n){
 lli l=lower_bound(all(v),p[i].ff)-v.begin();
 lli r=lower_bound(all(v),p[i].ss+1)-v.begin()-1;
