...
while(getline(cin,s)){
    ...
    if(vv.size()==3){
        // out(vv);
        if(vv[0]>vv[1]){
         st.update(1,0,n-1,vv[0],n-1,vv[2]);
         st.update(1,0,n-1,0,vv[1],vv[2]);
        }else
        st.update(1,0,n-1,vv[0],vv[1],vv[2]);

    }else if(vv.size()==2){
        // out(vv);
        if(vv[0]>vv[1]){
           cout<<min(st.quer(vv[0],n-1),st.quer(0,vv[1]))<<'\n';
        }else
        cout<<st.quer(vv[0],vv[1])<<'\n';
    }
}
