...
class Segment_Tree{
   ...
   public:
    ...
    lli query_bs(lli id,lli l,lli r,lli lq,lli x){ //first element greater than or eqyual x in range lq till end
      push(id,l,r); 
      if(r<lq)return -1;
...
fr(i,k){
    ...
    if(t==1){
       ...
       c[a]=1-c[a];
       lli ch=c[a]-prev;
       st.update(1,0,n-1,a,n-1,ch);
    }else{
        cout<<st.query_bs(1,0,n-1,0,a+1)<<'\n';
    }
}
